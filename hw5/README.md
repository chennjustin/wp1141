...
- ✅ **手機版優化**
  ...
  - **漢堡選單**：
    - 手機版使用漢堡選單，按鈕位於導航欄左側（與 "For you"、"Following" tabs 同一排）
    - 點擊漢堡選單按鈕打開側邊欄，再次點擊或點擊 X 按鈕關閉
    - 手機版選單滑動顯示（從左側滑入），包含圖標和文字標籤
    - 選單打開時顯示半透明遮罩層，點擊遮罩可關閉選單
    - 側邊欄頂部顯示 "my X" 標誌，右側顯示 X 關閉按鈕（與標誌同一排）
    - 點擊導航項目後自動關閉選單
    - 桌面版保持固定側邊欄，不顯示漢堡選單按鈕
  - **主內容區域**：
    - 手機版佔滿寬度，桌面版保持 60% 寬度
    - 所有組件都添加了 rwd
