...
一個使用 React + TypeScript + Vite 開發的生存波次射擊遊戲。玩家在畫布中移動，系統會自動朝最近的敵人發射彈丸；清空一波之後可進入商店購買升級。

## 🎯 遊戲特色

- **自動射擊系統**：玩家只需移動，武器會自動瞄準最近的敵人
- **多種武器選擇**：三種不同特性的武器，各有優缺點
- **敵人波次系統**：隨著波次增加，敵人會變得更強
- **升級系統**：擊敗敵人獲得經驗值升級，提升能力
- **商店系統**：每波結束後可購買武器升級
- **豐富音效**：完整的音效系統，包含背景音樂和音效
- **流暢動畫**：角色和敵人的動畫效果
...
- **遊戲迴圈**：以 `<canvas>` + `requestAnimationFrame` 每幀更新與繪製
- **自動攻擊**：固定間隔找出射程內最近敵人，產生彈丸（含速度向量）
- **敵人 AI**：從地圖邊緣隨機出生，持續朝玩家移動；與玩家碰撞即 Game Over
- **波次系統**：清空當前波次敵人 → 進入商店 → 發放金錢 → 可購買升級 → 進入下一波
