...
/**
 * Select Tag page for editing transaction
 * 
 * This page allows users to change the tag (category) of an existing transaction.
 * After selecting a tag, the user is redirected back to the edit page with the new tag.
 */
export default function EditSelectTagPage() {
  ...
  // Get source page from URL parameters
  ...
  // Handle tag selection - redirect to edit page with new tag
  const handleTagSelect = (tagId: string, tagType: "EXPENSE" | "INCOME") => {
    router.push(`/wallets/${walletId}/transactions/${transactionId}/edit?tagId=${tagId}&type=${tagType}&from=${fromPage}`);
  };
