...
Coin Undergraduate 是一個從 0 開始的專案，一個記帳與分帳系統。本系統支援多人協作的錢包管理、自動分帳計算、訂閱服務自動記帳等功能，幫助使用者輕鬆管理個人與群體財務。
...
我是曾煥軒，在這次期末專案中，我主要負責分帳功能的開發、資料庫架構規劃，以及協助 review 團隊成員撰寫的程式碼。整個開發過程中，讓我最深刻的體會是  code review 的重要性 。如果一味依賴 Cursor 產生程式碼，到了專案後期往往會變得難以維護；相反地，若能一開始就清楚定義各個模組的  domain、repositories、services、routes ，並持續維護架構的一致性，整個專案的可讀性與可維護性都會大幅提升，這才是我們真正應該學習的重點。

我認為本次專案的另一個挑戰在於版本切分與工作量分配。由於我們有許多想實作的功能，因此將專案分為三個版本，分別在十一月中、十二月初與十二月底完成，但如何在有限時間內合理分配每個階段的工作量，對團隊來說是一個不小的考驗。

在技術層面上，我學到了許多關於資料庫設計與維護的實務經驗。實際開發時幾乎不可能一開始就將資料庫設計得百分之百完善，因此 migration 會頻繁發生，且還需要同時處理本地資料庫與 Neon 雲端資料庫的同步問題，若流程沒有規劃好，真的會浪費大量時間。

總結來說，透過這次專案，我不僅更加熟悉了 Next.js、Prisma、PostgreSQL 等技術，更重要的是學會如何設計一個  可擴展、易維護的系統架構 。這讓我深刻體會到，好的程式碼不只是「能跑」，而是要讓他人也能輕易理解與接手。這次專案為我未來的全端開發奠定了相當扎實的基礎，也非常感謝團隊成員的協作與支持，讓專案能夠順利完成。

---

### **學號：B12705017**

我是曾文儀，在本次專案中，我主要負責新增錢包、好友系統、統計資料頁面以及 UI 設計相關的工作。關於 UI 設計，我有相當多的體會。一開始我們先用 iPad 繪製介面雛形，但實際將設計交給 Cursor 產生畫面後，發現成果與預期落差很大，整體視覺相當不理想。最後我們參考了多款市面上的記帳軟體，並自行重新調整配色、排版與互動細節，才逐步做出現在這個較為成熟、耐看的版本。

這個過程讓我深刻體會到設計在產品中的重要性，UI/UX 不只是外觀好不好看，而是會直接影響使用者是否願意長期使用一個產品。

在功能開發方面，我花了大量時間處理錢包相關的邏輯，包含新增錢包、修改錢包、以及錢包與使用者之間的關係。由於錢包幾乎是整個記帳系統的核心概念，不同錢包就像不同的實體，且還支援多人共同使用，使得 CRUD 邏輯變得更加複雜。因此，我在這個過程中大量使用並學習了 `useState`、`useEffect`、`useMemo`、`useCallback` 等 React hooks，同時也更深入理解單人錢包與多人錢包在資料結構與操作流程上的差異。

總體而言，這次專案大幅提升了我的前端與系統設計能力，也讓我對產品設計與使用者體驗有了更全面的思考。非常感謝團隊成員的合作，讓整個開發過程能夠順利進行。

---

### **學號：B12705018**

我是陳竑齊，在本次期末專案中，我主要負責系統整合、新增與修改 transaction 與 subscription 的功能、通知系統的實作，以及整體部署流程。我在這次專案中學習到最多的是  系統架構的整體規劃能力 。

其中，通知系統是本次專案相當重要的一個功能，除了需要整合 Pusher 即時通訊服務外，還必須與 transaction 與 subscription 的狀態緊密配合，確保在正確的時間點發送通知，並維持系統的穩定性。

訂閱功能本身也是一個相當複雜的模組，涉及時間計算、系統自動建立 transaction，以及在訂閱更新或刪除時同步調整相關紀錄與通知，這些都需要謹慎設計，否則很容易產生資料不一致的問題。

我認為這次專案讓我收穫最多的是  模組化設計的實務經驗 。我們團隊中有成員在這方面經驗豐富，看到他將整個系統架構整理得清楚且一致，讓功能擴充與維護變得非常順利，也讓我深刻體會到良好架構對團隊協作的重要性。

透過這次專案，我學會如何設計一個可擴展、易維護的系統，同時也更理解團隊合作與專案管理在實務開發中的關鍵角色，這對我未來的開發經驗有非常大的幫助。
