...
class ProgressBarAnimation {
  ...
  private animateProgressBars(): void {
    this.progressBars.forEach((bar: any, index: number) => {
      setTimeout(() => {
  ...
  private animateProgressBar(element: any): void {
...
class ClipboardHelper {
  ...
  private setupCopyButtons(): void {
    this.copyButtons.forEach((button: any) => {
      button.addEventListener('click', (e: Event) => {
        e.preventDefault();
        const contactItem = button.closest('.contact-item');
        const copyText = (contactItem as any)?.getAttribute('data-copy');
  ...
  private showCopyFeedback(button: any): void {
