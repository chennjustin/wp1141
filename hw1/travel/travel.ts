...
class DestinationNavigation {
  ...
  private setupDestinationCards(): void {
    ...
    destinationCards.forEach((card: any) => {
...
class TimelineAccordion {
  private currentOpenDay: number | null = null;
...
function goBackToDestinations(): void {
  ...
  travelDetailPages.forEach((page: any) => {
...
document.addEventListener('keydown', (e: KeyboardEvent) => {
  if (e.key === 'Escape') {
    ...
    if (activeContent) {
      ...
      const dayNumber = parseInt((timelineBlock?.getAttribute('data-day') as string) || '0');
      if (dayNumber > 0) {
